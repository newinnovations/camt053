...
use schema::{Balance, BalanceTypeCode, Document, Entry, Statement};
use std::{
    fs::File,
...
/// Parses a camt.053 XML file at `source` into the raw `schema::Document`.
///
/// This is the full, verbose ISO 20022 representation; most callers should
/// prefer [`crate::SimpleStatement::load`] instead.
pub fn import_camt(source: impl AsRef<Path>) -> Result<schema::Document, CamtError> {
    let file = File::open(source)?;
    let reader = BufReader::new(file);
    import_camt_from_reader(reader)
}

/// Parses a camt.053 XML document from any buffered reader (e.g. a `.zip`
/// entry or an in-memory buffer) into the raw `schema::Document`.
pub fn import_camt_from_reader<R: BufRead + Read>(
    reader: R,
) -> Result<schema::Document, CamtError> {
  ...
  <BkToCstmrStmt>
    ...
    <Stmt>
      ...
      {balances_xml}
        ...
        import_camt_from_reader(xml.as_bytes()).expect("valid test fixture")
    }
